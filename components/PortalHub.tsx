...
const protection = "PROTECCIÓN".split("");
  ...
  return (
    <main className="gallery-home" onWheel={handleWheel}>
      ...
      <section id="galeria" className="gallery-stage" aria-label="Áreas de protección de Alleanza">
        <motion.div key={`palabra-${active}`} className="gallery-display-word" aria-hidden="true">
          {protection.map((letter, index) => <motion.span key={`${letter}-${index}`} initial={{ y: 0, rotate: 0 }} animate={reduceMotion ? undefined : { y: [0, index % 2 ? 18 : -16, 0], rotate: [0, index % 2 ? 1.5 : -1.5, 0] }} transition={{ duration: 1.05, delay: index * .035, ease: [.16, 1, .3, 1] }}>{letter}</motion.span>)}
        </motion.div>
        <motion.div className="gallery-orbit" animate={reduceMotion ? undefined : { y: [0, -15, 8, 0], rotateX: [0, 1.8, -.8, 0] }} transition={{ duration: 1.05, ease: [.16, 1, .3, 1] }} key={`orbita-${active}`} onPointerDown={(event) => setDragStart(event.clientX)} onPointerUp={(event) => { if (dragStart === null) return; const delta = event.clientX - dragStart; if (Math.abs(delta) > 45) move(delta < 0 ? 1 : -1); setDragStart(null); }} onPointerCancel={() => setDragStart(null)}>
      ...
      <nav className="gallery-options" aria-label="Seleccionar una categoría">
        {gallery.map((item, index) => <button key={item.label} type="button" className={index === active ? "is-active" : ""} aria-current={index === active ? "true" : undefined} onClick={() => setActive(index)}><span>{String(index + 1).padStart(2, "0")}</span>{item.label}</button>)}
      </nav>
