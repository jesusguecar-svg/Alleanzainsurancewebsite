...
export default function HealthExperience({ reviews, reviewsUrl }: { reviews: GoogleReviews | null; reviewsUrl?: string }) {
  ...
  return <div className={s.page}>
    ...
    <main id="contenido">
      ...
      <section id="alianzas" className={s.alliances} aria-labelledby="alliances-heading"><div className={s.sectionTop}><span className={s.eyebrow}>04 / ALIANZAS QUE SUMAN</span><ShieldCheck size={19} /></div><div className={s.allianceIntro}><h2 id="alliances-heading">Bien acompañado.<br /><em>Mejor protegido.</em></h2><p>Te orientamos para comparar opciones<br />de cobertura en cada etapa de tu vida.</p></div><div className={s.partnerGrid}><article className={s.partner}><div className={s.partnerBrand}><span className={s.globeSymbol} aria-hidden="true">◎</span><div><strong>Globe Life</strong><span>FAMILY HERITAGE DIVISION</span></div><ArrowUpRight size={23} /></div><p>Protección complementaria para afrontar el impacto económico de eventos cubiertos, con la orientación de Alleanza Insurance Corp.</p><a href="https://www.familyheritagegl.com/" target="_blank" rel="noopener noreferrer">Conoce Family Heritage <ArrowUpRight size={15} /></a></article><article className={s.partner}><div className={s.partnerBrand}><span className={s.acaSymbol}>+</span><div><strong>ACA Marketplace</strong><span>MERCADO DE SEGUROS MÉDICOS</span></div><ArrowUpRight size={23} /></div><p>Asesoría para explorar planes de Obamacare, entender sus beneficios y revisar las ayudas disponibles según tu elegibilidad.</p><a href="https://www.cuidadodesalud.gov/es/" target="_blank" rel="noopener noreferrer">Conoce el Mercado de Salud <ArrowUpRight size={15} /></a></article></div><p className={s.smallPrint}>Alleanza Insurance Corp. es una agencia de seguros independiente. El Marketplace es el mercado de seguros médicos, no una aseguradora. Las pólizas se emiten por las compañías aseguradoras correspondientes. Este sitio no pertenece al gobierno federal ni implica su respaldo.</p></section>
      ...
      <div className={s.closing}><span>ALLEANZA INSURANCE CORP. · AGENCIA INDEPENDIENTE</span><p>Conoce tus opciones.<br /><em>Decide con claridad.</em></p><a href="/">Explora el universo Alleanza <ArrowRight size={16} /></a></div>
    </main>
    <SiteFooter editorial />
    <a className={s.floatingWhatsapp} href={healthWhatsapp} target="_blank" rel="noopener noreferrer" aria-label="Habla con Alleanza Insurance Corp. por WhatsApp"><MessageCircle size={23} /><span>¿Hablamos?</span></a>
  </div>;
}
